...
public class AStar extends Graph {
    ...
    public AStar(int[][] matrix, Node initialNode, Node finalNode) {
        setInitialNode(initialNode);
        setFinalNode(finalNode);
        this.searchArea = new Node[row][col];
    ...
    public void setBlocks(int[][] test1Matrix) {
        for (int i = 0; i < test1Matrix.length; i++) {
            int row = test1Matrix[i][0];
            int col = test1Matrix[i][1];
            setBlock(row, col);
        }
    }
    ...
    private boolean isEmpty(PriorityQueue<Node> openList) {
     ...
     private void setBlock(int row, int col) {
        this.searchArea[row][col].setVisited(true);
    }
